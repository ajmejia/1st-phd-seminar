...
sabemos la probabilidad $Pr(\theta)$ de que un evento $\theta$ ocurra, ¿cuánta información obtenemos
si observamos que el evento ocurre? Bueno, exploremos las reglas que debe cumplir la función de
información $I[Pr(\theta)]$.
%
\begin{description}
%
\item[Rango:] Por conveniencia se impondrá la restricción de que la información debe ser una
cantidad tal que $I\geq0$.
%
\item[Monotonía y continuidad:] El dominio de $I$ viene representado por el rango $Pr=[0,1]$. Si se
tiene la certeza de que un evento ocurre, entonces la información obtenida por tras su observación
es $I(1)=0$. Mientras menor es la probabilidad de observar un evento, la información obtenida tras
su observación es mayor. Por lo tanto $I$ debe ser una función decreciente en $Pr$.
%
\item[Aditividad:] Si dos eventos independientes tienen probabilidad $Pr(\theta_1)$ y $Pr(\theta_2)$
de ocurrir, entonces su probabilidad conjunta es el producto de sus probabilidades individuales y la
información provista por la observación de ambos eventos es $I_{\theta_1,\theta_2} =
I(Pr(\theta_1)) + I(Pr(\theta_2))$.
%
\end{description}
...
%
$$H(Pr) \equiv -\int Pr\log{Pr}\,\text{d}\theta.$$
%
...
entropía? Uno puede demostrar fácilmente que la entropía tiene un máximo usando la desigualdad de
Gibbs:
%
$$\int Pr\,\log{\left[\frac{1/(\theta_\text{max}-\theta_\text{min})}{Pr}\right]}\text{d}\theta\leq0,$$
%
con la igualdad si y solo si $Pr=1/(\theta_\text{max}-\theta_\text{min})$.
...
Supongamos que tenemos información contrastable sobre una hipótesis $\theta$ y queremos asignar una
distribución de probabilidad que cumpla con esa condición. El método de máxima entropía consiste en
...
contrastable encapsulada en las funciones $g_i(Pr;\theta,I)$, normalizadas por los multiplicadores
de Lagrange $\lambda_i$.
